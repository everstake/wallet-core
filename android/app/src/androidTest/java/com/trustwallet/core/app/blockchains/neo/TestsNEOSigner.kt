// Copyright ? 2017-2020 Trust Wallet.
//
// This file is part of Trust. The full Trust copyright notice, including
// terms governing use, modification, and redistribution, is contained in the
// file LICENSE at the root of the source code distribution tree.

package com.trustwallet.core.app.blockchains.neo

import com.trustwallet.core.app.utils.Numeric
import org.junit.Test
import wallet.core.jni.NEOSigner
import wallet.core.jni.proto.NEO
import com.trustwallet.core.app.utils.toHexBytesInByteString
import junit.framework.Assert.assertEquals

class TestNEOSigner {

    init {
        System.loadLibrary("TrustWalletCore")
    }

    @Test
    fun NEOTransactionSigning() {
        val NEO_ASSET_ID = "9b7cffdaa674beae0f930ebe6085af9093e5fe56b34a5c220ccdcf6efc336fc5"
        val GAS_ASSET_ID = "e72d286979ee6cb1b7e65dfddfb2e384100b8d148e7758de42e4168b71792c60"

        val signerBuilder = NEO.SigningInput.newBuilder().apply {
            privateKey = "F18B2F726000E86B4950EBEA7BFF151F69635951BC4A31C44F28EE6AF7AEC128".toHexBytesInByteString()
            fee = 12345
            gasAssetId = GAS_ASSET_ID
            gasChangeAddress = "AdtSLMBqACP4jv8tRWwyweXGpyGG46eMXV"
        }

        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("c61508268c5d0343af1875c60e569493100824dbdba108b31789e0e33bcb50fb".toHexBytesInByteString())
                    .setPrevIndex(1)
                    .setAssetId(GAS_ASSET_ID)
                    .setValue(98899890000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("4eb2f96937a0d4dc96b77ba69a29e1de9574cbd62b16d881f1ee2061a291d70b".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("3fee0109d155dcfab272176117306b45b176914c88e8c379933c246a9e29ea0b".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("6ea9ce8c578bfeeecdf281f498e2a764689df3b93d6855a3cc45bd6b5213c426".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(400000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("f75ad3cbd277d83ee240e08f99a97ffd7e42a82a868e0f7043414f6d6147262b".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("054734e98f442b3e73a940ca8f594859ece1c7ddac14130b0e2f5e2799b85931".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("8b0c42d448912fc28c674fdcf8e21e4667d7d2133666168eaa0570488a9c5036".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(1)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(2)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(3)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(4)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(5)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(6)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(7)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(8)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(9)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("cf83bce600626b6077e136581c1aecc78a0bbb7d7649b1f580b6be881087ec40".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("9bd7572ba8df685e262369897d24f7217b42be496b9eed16e16a889dd83b394e".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("b4ee250397dde2f1001d782d3c803c38992447d3b351cdc9bf20cfaa2cbf995b".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("e1019ca259a1615f77263324156a70007b76cb4f26b01b2956b8f85e6842ac62".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("bd379df2aca526ac600919aaba0e59d4a1ad4e2f22d18966063cf45e431d016f".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("164c3f843b9b7bfa6a7376a1548f343acb5cdfa0193b8f31e8c9a647ea63ea7d".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("4acec74a76161eafe70e0791b1f504b5ba1d175fd4f340d5bf56804e25505e92".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(300000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("895c6629a71c84cbdc8956abea9ca2d9d215e909e6173b1a1a96289186a67796".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("54828143c4c3a0e1b09102e4ed29220b141089c2bc4200b1042eeb12e5e49296".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("5345e4abc86f7ace47112f5a91c129175833bafcaf9f1e1bcbbaf4d019c1c69d".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("c83e19d0d4210df97b3bc7768dc7184ae3acfc1b5b3ac9b05d2be0fe5a636b9f".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("3456b03f5cb688ce26ab1d09b7a15799136c8c886ca7c3c6bcb2363e61bb1bb1".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("048f73d6cc82d9d92b08044eccef66c78a0c22e836988ed25d6f7ffe24fb5b38".toHexBytesInByteString())
                    .setPrevIndex(10)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(34000000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("e5a7887521b8b3aaf2d5426617ddabe8ef8ea3eab31c80a977c3b8f339df5be0".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("1455e9dd3cd6a04d81cd47acc07a7335212029ebbdcd0abc3e52c33f8b77f6eb".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("da711260085211b5573801d0dfe064235c69e61a55f9c15449ac55cc02b9adee".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("04486cfed371103dd51a89205b2c8bcc45ad887c49a768a62465f35810437bef".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        run {
            val input = NEO.TransactionInput.newBuilder()
                    .setPrevHash("a5f27055a442db0e65103561900456d37af4233267960daded870c1ab2219ef4".toHexBytesInByteString())
                    .setPrevIndex(0)
                    .setAssetId(NEO_ASSET_ID)
                    .setValue(500000000)
                    .build()
            signerBuilder.addInputs(input)
        }
        val output = NEO.TransactionOutput.newBuilder()
            .setAssetId(NEO_ASSET_ID)
            .setAmount(25000000000)
            .setToAddress("Ad9A1xPbuA5YBFr1XPznDwBwQzdckAjCev")
            .setChangeAddress("Ad9A1xPbuA5YBFr1XPznDwBwQzdckAjCev")
            .build()
        signerBuilder.addOutputs(output)

        val plan = NEOSigner.sign(signerBuilder.build())
        val result = NEOSigner.sign(signerBuilder.build(), plan).encoded.toByteArray()
        val hex = Numeric.toHexString(result, 0, result.size, false)

        //  https://testnet-explorer.o3.network/transactions/0x7b138c753c24f474d0f70af30a9d79756e0ee9c1f38c12ed07fbdf6fc5132eaf
        assertEquals(
            "",
            hex)
    }
}
